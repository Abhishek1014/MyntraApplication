...
@RequestMapping("/brand") public class BrandController {
	...
	@RequestMapping(method = RequestMethod.GET, path = "/name/{brand_name}",
			produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ResponseEntity<BrandListResponse> brandsByName(@PathVariable(name = "brand_name", required = false)
																  String brandName) throws BrandNotFoundException {
		List<BrandEntity> brandEntityList = brandService.brandsByName(brandName);

		return populateBrandList(brandEntityList);
	}
